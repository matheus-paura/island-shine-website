 ...
 *    logo's own navy background — used in the header, footer, and as the
 *    source for the site's favicon / apple touch icon.
 *  - full-logo.png: icon + wordmark trimmed of empty canvas — used for the
 *    Open Graph / share image.
 *
 * Crop boundaries below were measured by scanning the source PNG for
 * non-background pixels (see conversation history) — re-run the same
 * measurement if the source logo file changes.
 *
...
async function main() {
  ...
  // Icon-only crop (mountain + palms), a little padding around the measured
  // bounding box (x:256-1718, y:342-1030), then padded to a square.
  await sharp(source)
    .extract({ left: 206, top: 292, width: 1562, height: 723 })
